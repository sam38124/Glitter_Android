...
class Glitter {
    constructor() {
        ...
        this.setNavigation = function (src, obj) {
            window.history.pushState({}, document.title, "?page="+glitter.iframe[glitter.iframe.length-1].id+"");
            if (window.drawer === undefined) {glitter.addScript('glitterBundle/NaviGation.js',function (){})}
        ...
        this.openNavigation = function () {
            if(window.drawer!==undefined){
                ...
                var timer=setInterval(function (){
                    if(window.drawer!==undefined){window.drawer.open();clearInterval(timer);}
                },100)
            }
        }
        //關閉側滑選單
